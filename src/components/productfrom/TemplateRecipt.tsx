  ...
  const calculateGSTAmounts = () => {
    ...
    saleDetails.products.forEach((product) => {
      if (product.gstnumber) {
        ...
        const gstAmount = parseFloat(((product.totalAmount * gstRate) / 100).toFixed(2)); // Ensure float precision
        totalCGST += gstAmount;
        totalSGST += gstAmount;
      }
    });
  
    return { totalCGST: parseFloat(totalCGST.toFixed(2)), totalSGST: parseFloat(totalSGST.toFixed(2)) };
  };
  
  // Use stored GST amounts if available, otherwise calculate
  const { totalCGST, totalSGST } =
    saleDetails.cgstAmount !== undefined && saleDetails.sgstAmount !== undefined
      ? { totalCGST: saleDetails.cgstAmount, totalSGST: saleDetails.sgstAmount }
      : calculateGSTAmounts();
  
  // Calculate saved amount with float precision
  const calculateSavedAmount = () => {
    return parseFloat(
      saleDetails.savedAmount !== undefined
        ? saleDetails.savedAmount.toFixed(2)
