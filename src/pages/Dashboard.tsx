...
//           new Date(
//             Math.max(
//               ...stats.recentSales.map((s: any) => new Date(s.saleDate).getTime())
//             )
//           ).toLocaleDateString()
//       )
//       .map((sale: any) => (
...
export default function Dashboard() {
  ...
  // Determine the latest sale date
  const latestSaleDate = new Date(
    Math.max(
      ...stats.recentSales.map((sale: any) => new Date(sale.saleDate).getTime())
    )
  ).toLocaleDateString();

  // Filter recentSales to only include records matching the latest sale date
  const latestSales = stats.recentSales.filter(
    (sale: any) =>
      new Date(sale.saleDate).toLocaleDateString() === latestSaleDate
  );
